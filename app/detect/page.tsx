...
export default function DetectPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {image && (
        <div className="mb-8">
          <h2 className="text-2xl font-semibold mb-4 text-secondary">Uploaded Image</h2>
      ...
      {results && (
        <div>
          <h2 className="text-2xl font-semibold mb-4 text-secondary">Detection Results</h2>
          <ul className="list-disc pl-5 text-gray-300">
            {results.map((result, index) => (
              <li key={index} className="mb-2">
                <span className="text-primary">{result.label}:</span> {(result.confidence * 100).toFixed(2)}% confidence
              </li>
            ))}
          </ul>
        </div>
